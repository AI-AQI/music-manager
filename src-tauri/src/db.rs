...
fn seed_standard_tags(conn: &Connection) -> Result<(), String> {
    ...
    for category in ["情绪", "节奏", "用途", "人声", "自定义"] {
    ...
    for name in ["治愈", "温暖", "激昂", "紧张", "悬疑", "悲伤", "浪漫", "舒缓", "中速", "快节奏", "Vlog", "纪录片", "剧情", "广告", "转场", "片头", "片尾", "纯音乐", "人声", "旁白友好"] {
...
pub(crate) fn tag_category(name: &str) -> &'static str {
    match name {
        "治愈" | "温暖" | "激昂" | "紧张" | "悬疑" | "悲伤" | "浪漫" => "情绪",
        "舒缓" | "中速" | "快节奏" => "节奏",
        "Vlog" | "纪录片" | "剧情" | "广告" | "转场" | "片头" | "片尾" => "用途",
        "纯音乐" | "人声" | "旁白友好" => "人声",
        _ => "自定义",
    }
}
